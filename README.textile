...
SmokeSignals::Condition.handle(MyCondition1 => lambda {|c| c.restart(:some_restart),
                               MyCondition2 => lambda {|c| c.restart(:another_restart) }) do }) do
  # Stuff that signals...
end
</code></pre>
...
<pre><code>SmokeSignals::Condition.handle(lambda {|c| puts 'this is run 2nd' }) do
  SmokeSignals::Condition.handle(lambda {|c| puts 'this is run 1st' }) do
    begin
      SmokeSignals::Condition.new.signal
...
<pre><code>def parse_file(filename)
  SmokeSignals::Condition.with_restarts(:use_new_filename => lambda {|f| parse_file(f) }) do
    file = nil
    begin
