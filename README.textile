...
However, you're probably using some code that doesn't know about SmokeSignals and raises exceptions instead.  Setting a condition handler will not handle these raised exceptions.  They couldn't because in such a case, restarting would be impossible.  By the time an exception is rescued, the stack has already been unwound.
